...
- 使用半透明 `--dsw-specific-menu` 填充的高层级表面，必须在同一规则中应用 `backdrop-filter: var(--dsw-menu-backdrop-filter)`。共享下拉菜单采用紧凑 `Menu` 几何；功能包自有菜单内容可以不同，但不得重新实现材质或滚动条（[决定](../.agents/notes/implemented/feature/2026-09-17-compact-translucent-menu-surfaces.zh.md)）。
