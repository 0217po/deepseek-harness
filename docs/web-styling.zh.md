...
- 存量和新增的下拉菜单、上下文菜单、子菜单及选择菜单使用 `Menu`，自定义内容用 `MenuSurface` 包裹。默认材质由主题的 `--dsw-specific-menu` 填充和 `--dsw-menu-backdrop-filter` 组成；功能及平台 CSS 不得覆盖两者。只有显式使用 `MenuSurface material="solid"` 并在调用处说明原因的菜单可以省略模糊。ui-theme 菜单检查拒绝未使用共享容器的 menu/listbox 渲染、材质覆盖和 token 重定义。
- 设置、确认框等模态弹窗保留黑色半透明遮罩，不模糊背景。主题将 `--dsw-mask-blur` 设为 `none`；Desktop 自有模态浮层也不对父页面施加模糊。
- 菜单背景模糊绘制在独立图层上，保证嵌套菜单和 fixed 浮层的定位及背景采样。macOS 上，`MenuSurface` 仅在各菜单范围内、页面内容后方添加实色底层，使 Chromium 能在原生透明材质上模糊页面内容；CSS 锚点跟随位置及尺寸，卸载时移除底层。其他使用菜单填充的浮起表面也须配套同一模糊；包含 fixed 浮层时，将两者绘制在独立背景层上（[决策](../.agents/notes/implemented/feature/2026-09-17-compact-translucent-menu-surfaces.zh.md)）。
