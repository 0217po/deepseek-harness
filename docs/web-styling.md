...
- Existing and new dropdown, context, submenu, and selection menus use `Menu` or wrap custom content in `MenuSurface`. The default material is the theme-owned `--dsw-specific-menu` fill plus `--dsw-menu-backdrop-filter`; feature and platform CSS must not override either. Only an explicit `MenuSurface material="solid"` with a caller-local reason may omit blur. The ui-theme menu spec rejects unwrapped menu/listbox renderers, material overrides, and token redefinitions.
