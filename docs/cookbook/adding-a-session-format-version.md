...
Released codecs and accepted target-format meanings remain stable. The N→N+1 converter may receive fixes or support additional historical cases after N+1 ships, without a version bump, if its output remains compatible with N+1. Changing the established target representation incompatibly requires the next version; improving the incoming conversion does not.
...
Use the `plugin:` convention for historical unknown identifiers and conflicting extension-field names. Preserve complete original names and payloads, distinguish source and field owners to prevent collisions, and leave known core identities and opaque nested data alone. Document each exact mapping in the edge README.
