...
已发布 codec 和已接受的目标格式含义保持稳定。N+1 发布后，N→N+1 转换器仍可修复或增加历史场景支持；只要输出仍兼容 N+1，就不必提高版本。不兼容地改变既定目标表示需要下一版本，改进传入转换本身不需要。
...
历史未知标识符和冲突的扩展字段名使用 `plugin:` 约定。保留完整原名称与载荷，区分来源及字段 owner 以防冲突；不改变已知核心身份或不透明嵌套数据。每个精确映射写入迁移边 README。
