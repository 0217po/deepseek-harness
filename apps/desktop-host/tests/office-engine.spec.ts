...
function fixture() {
  ...
  roots.push(root)
  const runtime = join(root, 'app.asar', 'dsh')
  const manifest = 'node_modules/@deepseek-ai/libreoffice-kit-darwin-arm64/package.json'
  for (const base of [runtime, join(root, 'app.asar.unpacked', 'dsh')]) {
    const path = join(base, manifest)
    mkdirSync(dirname(path), { recursive: true })
    writeFileSync(path, '{"name":"@deepseek-ai/libreoffice-kit-darwin-arm64"}')
  }
...
it('resolves engine manifests to physical directories and leaves unrelated modules alone', () => {
  const f = fixture()
  expect(f.require.resolve('@deepseek-ai/libreoffice-kit-darwin-arm64/package.json'))
    .toBe(realpathSync(join(f.root, 'app.asar.unpacked', 'dsh', f.manifest)))
  expect(f.require.resolve('node:fs')).toBe('node:fs')
  expect(f.require.resolve('@deepseek-ai/libreoffice-kit/package.json'))
    .toBe(realpathSync(join(f.runtime, 'node_modules/@deepseek-ai/libreoffice-kit/package.json')))
})
...
it('rejects an engine missing from the unpacked tree instead of using its archived copy', () => {
  ...
  expect(() => f.require.resolve('@deepseek-ai/libreoffice-kit-darwin-arm64/package.json')).toThrow()
})
