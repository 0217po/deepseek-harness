...
describe.skipIf(MODE === 'record')('web e2e: details panel follows the current Session lifecycle', () => {
  ...
  beforeAll(async () => {
    ...
    // The Desktop preload marks the platform before client plugins render.
    ...
    const leading = page.locator('[data-conversation-header-leading]')
    await leading.getByRole('button', { name: 'Open sidebar', exact: true }).waitFor({ state: 'visible' })
    expect(await leading.getByRole('button', { name: 'New session', exact: true }).isVisible()).toBe(true)
    expect(await page.getByRole('tablist').count()).toBe(0)
    await leading.getByRole('button', { name: 'Open sidebar', exact: true }).click()
