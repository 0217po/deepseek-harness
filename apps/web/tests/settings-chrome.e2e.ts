...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('persists the completed-Turn transcript mode across reload', async () => {
    ...
    await dialog.getByRole('button', { name: '简洁', exact: true }).click()
    await page.getByRole('menuitem', { name: '完全展开', exact: true }).click()
    await dialog.getByRole('button', { name: '完全展开', exact: true }).waitFor({ timeout: 10_000 })
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
    ...
    await reloaded.getByRole('button', { name: '完全展开', exact: true }).waitFor({ timeout: 10_000 })

    await reloaded.getByRole('button', { name: '完全展开', exact: true }).click()
    await page.getByRole('menuitem', { name: '简洁', exact: true }).click()
    await reloaded.getByRole('button', { name: '简洁', exact: true }).waitFor({ timeout: 10_000 })
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
