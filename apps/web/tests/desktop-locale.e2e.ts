...
describe.skipIf(MODE === 'record')('web e2e: native and Client locale preferences', () => {
  ...
  beforeAll(async () => {
    ...
    await page.exposeFunction('nativeLocaleRead', () => {
      const section = scaffold.ctx.settings.get('locale') as { preference?: string }
  ...
  it('uses OS languages without saving them, then shares an explicit choice across reloads', async () => {
    ...
    expect(scaffold.ctx.settings.get('locale')).toEqual({})
    ...
    await expect.poll(() => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'))
      .toMatch(/locale:\n\s+preference: en/u)
