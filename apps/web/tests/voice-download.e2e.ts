...
it.each([0, 1])('starts at responsive source %s and reports the fallback source through the real preparation UI', async (winner) => {
  ...
  const alert = page.getByRole('alert').filter({ hasText: 'Cannot download model.int8.onnx' })
  await alert.waitFor()
  expect(downloads).toEqual([winner, 1 - winner])
  ...
  expect(await alert.textContent()).toContain('HTTP 502')
  expect(await alert.textContent()).toContain(`Download source: ${origins[1 - winner]}`)
  await compareOrRefreshGolden(fileURLToPath(new URL('./expected/voice-download-fallback.expected.md', import.meta.url)),
    (await captureStableAria(page, '[role="alert"]', scaffold.workspaceCwd)).replaceAll(origins[1 - winner]!, 'https://fallback-model-source.example'),
    webSnapshotMode())
  expect(tripwire.pageErrors).toEqual([])
})
