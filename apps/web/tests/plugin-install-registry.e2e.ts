...
it('offers the registries, remembers the one picked, and moves an install on to the next when a run loses its registry', async () => {
  ...
  try {
    ...
    try {
      ...
      const registryToggle = dialog.getByRole('button', { name: '安装源 默认安装源（registry.npmjs.org）', exact: true })
      ...
      await dialog.getByRole('button', { name: '安装源 中国大陆镜像源（registry.npmmirror.com）', exact: true }).waitFor()
      ...
      await dialog.getByText('第 1 次 · 中国大陆镜像源（registry.npmmirror.com）', { exact: true }).waitFor()
      await dialog.getByText('第 2 次 · 默认安装源（registry.npmjs.org）', { exact: true }).waitFor()
      ...
      await panel.getByRole('button', { name: '添加插件', exact: true }).click()
      await page.getByRole('dialog').getByRole('button', { name: '安装源 中国大陆镜像源（registry.npmmirror.com）', exact: true }).waitFor()
