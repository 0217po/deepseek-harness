...
it('boots default Web without experimental modules or an active built-in Browser', async (test) => {
  await withDefaultWeb(test, async ({ url, request }) => {
    ...
    expect(roster.entries).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ id: 'time-context', name: '@deepseek-ai/dsh-time-context', state: FiberState.ACTIVE }),
      expect.objectContaining({ id: 'schedule', name: '@deepseek-ai/dsh-schedule', state: FiberState.ACTIVE }),
      expect.objectContaining({ id: 'ui-schedule', name: '@deepseek-ai/dsh-client-ui-schedule', state: FiberState.ACTIVE }),
    ]))
