...
要在已有 Desktop 旁边启动第二个实例，请双击 [`启动Desktop.command`](../../启动Desktop.command)。该脚本从默认 Desktop 端口之后选择空闲端口，设置 `DSH_DESKTOP_WEB_PORT`，并使用 `~/.dsh-desktop-<仓库>-<端口>` 下的独立 `$DSH_HOME`，不会复用已有实例的状态和端口。
