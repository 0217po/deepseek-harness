...
int wmain(int argc, wchar_t** argv) {
    ...
    assert(uninstall_data::Remove(cache.c_str(), installation.c_str()) == ERROR_SHARING_VIOLATION);
    CloseHandle(held);
    assert(uninstall_data::Remove(cache.c_str(), installation.c_str()) == ERROR_SUCCESS);
    assert(GetFileAttributesW((data + L"\\Local State").c_str()) != INVALID_FILE_ATTRIBUTES);
    assert(uninstall_data::Remove(cache.c_str(), installation.c_str()) == ERROR_SUCCESS);
    assert(uninstall_data::Remove(data.c_str(), installation.c_str()) == ERROR_SUCCESS);
    assert(GetFileAttributesW(data.c_str()) == INVALID_FILE_ATTRIBUTES);
