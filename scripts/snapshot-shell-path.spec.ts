...
it.skipIf(process.platform === 'win32')('translates only the exact fixture command and preserves real shell failures', async () => {
  ...
  try {
    ...
    await fork
    const outcome = await ctx.shell.execute(ctx.shell.resolve({ command })).result()
    expect(outcome.timedOut).toBe(false)
    expect(outcome.exitCode).toBe(0)
    expect(await readFile(livePath, 'utf8')).toBe('actual bytes')
    const untouched = await ctx.shell.execute(ctx.shell.resolve({ command: "printf '/fixture/recorded.txt'; exit 7" })).result()
    expect(untouched.exitCode).toBe(7)
    expect(untouched.stdout.text).toBe('/fixture/recorded.txt')
    await fork.dispose()
    await rm(livePath)
    const restored = await ctx.shell.execute(ctx.shell.resolve({ command })).result()
