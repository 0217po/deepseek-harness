...
it.skipIf(process.platform === 'win32')('resolves the exact spill verifier, retains real failures, and restores the shell', async () => {
  ...
  try {
    ...
    const output = await ctx.shell.execute(spec).result()
    expect(output.timedOut).toBe(false)
    expect(output.exitCode).toBe(0)
    expect(output.stdout.text).toBe('SPILL_CANONICAL_OK\n')
    expect(spec.command).toBe(command)
    const other = await ctx.shell.execute(ctx.shell.resolve({ command: 'printf ordinary; exit 7' })).result()
    expect(other.exitCode).toBe(7)
    expect(other.stdout.text).toBe('ordinary')
    const unmatched = await ctx.shell.execute(ctx.shell.resolve({ command: command + '; exit 9' })).result()
    ...
    await rm(physical)
    const missing = await ctx.shell.execute(spec).result()
    expect(missing.exitCode).not.toBe(0)
    expect(missing.stdout.text).toBe('')
    await adapter.dispose()
    const restored = await ctx.shell.execute(spec).result()
