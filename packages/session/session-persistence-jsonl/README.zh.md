...
发布产物在新建的 Worker Thread 中运行打包后的 JavaScript。源码执行使用加载 ESM TypeScript 加载器的独立 Node 进程，避免在短生命周期线程内注册加载器。两条路径保持相同的校验、取消及最多两个隔离执行单元并发的限制；清理会等待所属线程或进程停止。
