...
`open(id, 'read'|'write')` 选择最高规范代次。当前输入走普通快速路径。对于历史输入，读 open 解码并迁移源文件一次，校验当前逻辑结果，不发布后继文件。写 open 复用按源版本缓存的准备结果，或重新准备，再分块写同目录临时文件，经 Worker Thread 校验、复核源文件版本后，以不覆盖方式发布当前后继文件。源文件字节保持不变。准备后的源文件变化会拒绝该次写 open，已返回的逻辑历史不变；后续写 open 重新准备。缓存前深度冻结的事件图标记为 `shared-frozen`，读取和切片（包括空切片）保持这一状态；只有尚未物化的待写日志报告 `detached`。`stat(id)` 和 `list()` 只选择并转换最高代次的头部，不读事件行或启动迁移；快照携带所选文件的 `sizeBytes` 和尽力获取的版本标识。版本标识只对应所选会话文件，与其他会话无关。配置 `compression: 'none'` 时，外部工具可以直接读取逐行文本；压缩默认格式必须经后端读取。

历史准备只读取并迁移所请求的会话。它保留已有父会话 catalog 事件，不收集子会话 descriptor，也不校验其他会话的头部或正文。子会话损坏、不支持的代次和子会话后继发布不会使父会话准备结果失效。[子代理服务](../../subagent/subagent/README.zh.md) 从头部发现缺失的子会话条目；打开选中的子会话时才独立迁移并报告其错误。当前读取在暴露事件前校验 catalog 字段、唯一性和交付归属。

历史版本标识和缓存复用不需要全库指纹或子会话正文扫描。列表仍枚举会话头部；定位尚未缓存的 id 时，定向代次查找可能枚举目录。只读访问不发布升级，因此冷进程和缓存淘汰后只重复所请求会话的解码。[诊断测量](../../../.agents/notes/implemented/architecture/2026-08-31-released-session-format-migrations.zh.md#catalog-scan-measurements) 保留此前主动扫描的基线。
