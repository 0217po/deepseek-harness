...
Published verifiers run the bundled JavaScript in fresh Worker Threads. Source execution uses isolated Node processes with the ESM TypeScript loader, avoiding loader registration inside short-lived threads. Both paths retain the same verification, cancellation, and two-isolate concurrency limit; teardown waits for the owned isolate to stop.
