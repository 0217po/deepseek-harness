...
The migration declaration creates independent streaming stages. Compact runs expand as iterables without an intermediate event array. The V4 codec uses the released V2 codec only for physical header and source-range framing, and validates native tool-role rows directly. JSONL scanners call `assertV4RowAdmission` before suppressing recoverable rows and the shared mandatory relationship validator before returning the completed logical prefix.
