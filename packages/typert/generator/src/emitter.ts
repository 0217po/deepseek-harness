...
/** Protocol entry that exports the handle a generated Client stream method returns. */
const REMOTE_CLIENT_PROTOCOL = '@deepseek-ai/dsh-typert-protocol/client'
...
export class FaceModelEmitter {
  ...
  private renderRemoteDts(packageModel: PackageModel): Pick<RemoteModelEmitResult, 'dts' | 'dtsMap'> {
    ...
    // A stream method's Client signature names the handle from the protocol's
    // Client entry; the Host entry's `RemoteStream` alias would shadow a
    // same-named import inside the augmentation below.
    const lines = [
      '/* Generated by @deepseek-ai/dsh-typert-generator from the Host FaceModel — do not edit. */',
      'import type {',
      '  RemoteResult,',
      ...
      ...packageModel.invocations.some(invocation => invocation.mode === 'stream')
        ? [`import type { RemoteStreamHandle } from ${quote(REMOTE_CLIENT_PROTOCOL)}`]
        : [],
    ]
...
function allocateRemoteImportNames(imports: readonly RemoteTypeImportModel[]): ReadonlyMap<SymbolId, string> {
  const used = new Set(['RemoteStreamHandle', 'TypertRemoteContribution', 'TYPERT_REMOTE'])
