...
- **后台 provider failure 提示就是整条 stderr 流**——`SubprocessHandle.done` 可能在 target 开始执行前或后 reject，而 subprocess 服务不会为从未上报结果的 target 缓冲任何输出，因此执行器把不声明失败阶段的 `subprocess failed before reporting an outcome: …` 作为 observed stderr 流提供（偏移读取方按各自偏移重读），并只折入恰好一个 `readOutput()` 增量；丢弃了该增量的消耗式读取方只能经 `observed.stderr` 恢复它。
