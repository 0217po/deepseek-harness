...
Client list rows and resident Sessions use the current `sessionListMetadata` projection to reject stale blank-session hints; recency is the later of the summary timestamp and the projected last user prompt. An opened conversation is therefore not reused by New Session even when an older list response still marks it blank.
