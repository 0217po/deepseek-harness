...
GUI 模型选择与提示词准入要求确切提供方／模型对出现在当前可用目录中。不可用的选择在投递前以 `session/model-unavailable` 拒绝；附件准备完成后再次检查。`initializeDefaultModel(provider)` 仅在用户尚未选择时，保存其显式配置的提供方的第一个可用模型。可用性变化不会替换模型或改写会话选择。
