...
On Windows, this package confines child-process writes and deletes to the workspace and a private temporary directory: `workspace-write` grants both, `read-only` grants neither. Mounting `dsh-sandbox-local` selects this for confined bash and PowerShell commands, or callers use the public `AclSandbox` API directly; any failed Win32 operation prevents an unrestricted spawn. Two layers enforce the boundary, the write-restricted token's capability-SID intersection and a Low integrity label on the granted directories, but the guarantee stays partial: process startup retains Everyone access, hard links alias file objects, and files ACL'd by another AppContainer tool stay unreadable.
...
- **Writes and deletes are restricted; reads, network, and process visibility are not** — neither layer intersects reads, so a confined child can read any caller-readable file and open sockets; `read-only` therefore needs a read-side policy to be expressed.
...
- **Security descriptor edits are standing directory mutations** — workspace ACEs and labels stand by design (the reuse cache, never revoked); temp edits are revoked by `dispose()`; manual `icacls` cleanup cannot revoke them on this platform (`ERROR_NONE_MAPPED`, 1332), so revoke through this module.
...
- **Reads stop at objects another AppContainer-based tool has ACL'd with a package SID** — Windows denies a Low-integrity token access to a file whose DACL carries an ACE for a package SID (`S-1-15-2-…`), even when that DACL also grants the user full control and Everyone read. Tools that sandbox themselves with AppContainers grant exactly such ACEs, so a project tree stamped by one of them becomes unreadable to this backend's child. Removing the foreign ACE (or re-installing the affected tree) restores access; the package cannot grant it back.
