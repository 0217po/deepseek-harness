...
The keyboard reference exposes New Session, Search sessions, Add workspace, Rename session, Fork session, and Archive session. Desktop defaults use the platform's primary modifier, with Mod+K for search; Windows and macOS Web use the [shortcut service’s platform defaults](../shortcuts/README.md); Linux Web leaves these commands unbound until configured. Button tooltips display the effective binding. Every Session row menu shows the effective Rename, Fork, and Archive shortcuts. Clicking a menu item targets that row; pressing its shortcut targets the main Session. Commands target the main Session. Windows Desktop bindings also execute from terminal input; other environments preserve terminal ownership. Modal dialogs block background commands. Search and rename requests belong to this package; input drafts remain in the browser. The directory picker rejects another open while selection or Workspace adoption is pending. Fork captures the latest completed turn before execution; if its ending is outside the loaded history, the owner pages backwards until it finds that turn or exhausts history. It stops after navigation, disposal, or a page without progress; the reference reports history loading while the target remains unknown.
