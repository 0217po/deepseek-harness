...
This package declares `UserQuestionPanels` on the browser `Context` for its question row: `reveal(sessionId, callId): boolean` shows that call's answer panel and reports whether one was left to show, and `review(sessionId, callId, record): boolean` shows a settled call's recorded questions and answers as a read-only panel, with the record read from this row's own recorded call plus whichever recorded its answers — the result, or the projection's late-reply entry. The question view reads both through `ctx.get('userQuestionPanels')` inside its slot `inject`, so a composition without question UI renders the same row with no action. `ui-user-questions` provides the capability and owns the panel behavior behind it.
