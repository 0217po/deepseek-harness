...
function props(
  ...
  return { sessionId: SESSION, useSessions, useJobOutput, observe, killJob, t } as unknown as JobListActionProps
}
...
describe('JobListAction rows', () => {
  ...
  it('clearing drops an expanded settled panel with its rows', () => {
    const observe = vi.fn(() => () => {})
    render(<JobListAction {...props(
      [recordJob({ id: 'bash-2' as SessionJob['id'], label: 'settled work', status: 'completed', finishedAt: 1_700_000_012_000 })],
      observe,
    )} />)
    openList()
...
describe('JobListAction human kill', () => {
  ...
  it('offers the stop control only on running rows', () => {
    render(<JobListAction {...props([
      job(),
      job({ id: 'bash-2' as SessionJob['id'], label: 'done', status: 'completed', finishedAt: 1_700_000_100_000 }),
    ])} />)
    openList()
