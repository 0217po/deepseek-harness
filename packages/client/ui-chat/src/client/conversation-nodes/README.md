...
| Failed | Keep process content open; whole-Turn collapse is unavailable. |
...
| Other closed Turn with loaded process content, including stopped Turns | Initially collapse the process; allow manual opening and closing. Neither the Turn start nor the rest of Session history must already be loaded. |
...
The top status and trigger title share the group-title 14px font size and follow the font-size setting. The top status has 16px clearance from the preceding input, and its divider has 16px clearance from the following content. The top status is absent while running. The clock updates in whole seconds, starts at one second, and uses hours from 60 minutes. Completion fixes the duration and shows “Completed in”; cancellation shows “Stopped”, and failure shows its status. While running, “Deep diving for” and the live duration follow the content and disappear on completion. Its text and icon are 2px smaller than the group title and icon, follow the font-size setting, and keep a 32px gap from adjacent replies. The status is centered between fixed 32px-long, 0.5px dividers using the l3 border color. Each divider keeps a 16px gap from the status. A running transcript has 24px of bottom padding before the composer. Duration digits use the code font with tabular numerals. A leading whale-tail Lottie animation sways in its native 3-second loop, once per two text sweeps, shares their 0.3-second initial delay, and keeps a 6px text gap. The icon and text use deep brand blue in the light theme and lighter, less saturated blue in the dark theme. The text sweep uses darker blue in the light theme and brighter blue in the dark theme. Reduced-motion preferences hold the whale tail at its initial pose and disable the text sweep. Lifecycle changes have a polite announcement; clock ticks do not.
...
Group headers show a category icon, replace it with a down arrow on hover or keyboard focus, and show an up arrow while open. Group bodies use 8px spacing between process rows, including reasoning and skill loading. Manually expanded groups have a `min(400px, 50vh)` height cap and 24px directional fades. Wheel scrolling can continue into the outer transcript at an edge. Expanded mode removes the group-level cap only in running Turns; process row spacing remains 8px, and spacing next to Assistant responses remains 16px.
