...
Every admitted request speaks for a Peer. `ctx.connection.peers` is the Host-wide `PeerRegistry`: `operator` is the process's own Peer; `open()` admits another party and returns a `PeerScope` whose `ctx` owns connection-lifetime registrations until `dispose()`; `bind(carrier, peer)` and `of(carrier)` associate a node request, upgrade request, WebSocket, or Fetch `Request` with the Peer it was admitted as. `ctx.connection.admit(request)` answers from a binding when one exists and otherwise runs the trust and authentication checks and answers with the operator; the `/api` route and the Gateway's WebSocket upgrade admit through it, and every RPC handler receives the Peer of its call. Who a Peer is and what it may do are not recorded here: the admitter attaches that through `peer.ctx` or a registry keyed by the scope.
