...
Settings visibility and section selection live in the shell owner store. Its trigger reads the effective command binding for hover and keyboard-focus hints and `aria-keyshortcuts`. Desktop Mod+, and the sidebar control open the same dialog; repeated commands keep it open. A nested shortcut reference closes before Settings on Escape, then restores its invoking control.
