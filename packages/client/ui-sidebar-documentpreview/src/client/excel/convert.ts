...
import { update as formatNumber, type Cell, type CellStyle, type Sheet, type SheetConfig } from '@fortune-sheet/core'
...
/** Parser failures with locale-independent categories. */
export class ExcelPreviewError extends Error {
  /** @param code - User-actionable parser failure. */
  constructor(readonly code: 'invalid' | 'tooLarge' | 'timeout', options?: ErrorOptions) {
    super(code, options)
 ...
 */
export async function convertExcel(bytes: Uint8Array<ArrayBuffer>, limits: ExcelLimits): Promise<ExcelPreview> {
...
function convertCell(
  ...
  if (result.ht === undefined) result.ht = typeof result.v === 'number' ? 2 : typeof result.v === 'boolean' ? 0 : 1
  if (result.ct === undefined) result.ct = { fa: cell.numFmt || 'General', t: typeof result.v === 'number' ? 'n' : typeof result.v === 'boolean' ? 'b' : 's' }
  const formatted = typeof result.v === 'number' ? formatNumber(cell.numFmt || 'General', result.v) as string | number : result.v
  result.m = formatted === undefined ? '' : String(formatted)
  return result
}
