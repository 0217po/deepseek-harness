...
Fixed stop input reads a package-internal current-turn source from the existing assembly. Observed turn changes publish synchronously even when no View is active; a turn is available only after its start is loaded. Unloading the Session binding detaches its event-source subscription.
