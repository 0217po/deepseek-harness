...
Width-handle pointer moves first check a synchronous dragging ref, then pointer capture, before reading layout, writing styles, or scheduling animation frames. Pointer handlers update the drag indicator directly without a React state update. The indicator follows the pointer only during a captured drag; ordinary hover leaves its position unchanged.
