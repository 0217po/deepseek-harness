...
`ctx.uiConversation.binding(binding).timeline` exposes the same immutable loaded Turn/Step timeline that target builders receive. Turn and Step lifecycle events publish synchronously, including when no View is active; unloading the Session binding detaches its source subscription. Consumers capture the selected Session and event sequence before an asynchronous command. The `historyStart` source publishes the oldest loaded event sequence even when a page contains no Turn or Step changes, so history consumers can detect pagination progress without reading raw events.
