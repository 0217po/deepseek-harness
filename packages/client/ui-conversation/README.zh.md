...
`conversation.input.activity` 在模型选择器与发送按钮之间承载一个控件。其 `onActiveChange` 回调将控件展开至整条工具栏并隐藏普通辅助控件，同时保留编辑器、上下文用量控件和提交按钮。上下文用量按钮在活动控件与发送按钮之间保留独立位置。占用者在卸载时释放展开状态，并拥有活动专属反馈。
