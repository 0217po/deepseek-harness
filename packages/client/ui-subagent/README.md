...
The session header keeps the current session title as the lineage breadcrumb; when the session has subagent descendants, the descendant-count trigger renders in the header actions band after the task list, with no breadcrumb separator. The trigger opens the descendant catalog, counts the complete subagent-only lineage, stops at ordinary forks, and shows ongoing activity when any counted descendant is running. Select any depth to open that child's conversation with its exact `{parentSessionId, childSessionId, mode}` address.
