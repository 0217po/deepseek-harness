...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    await configuration.update({ preference: 'dark', fontSize: 24 })
    expect(plainConfig(configuration.fiber.config)).toEqual({ preference: 'dark', fontSize: 24 })
    ...
    await expect(configuration.update({ fontSize: 25 })).rejects.toThrow()
    await fiber.dispose()
  })
  ...
  it('answers each collection with the current durable preference until disposal', async () => {
    ...
    expect(rowText(collect(ctx)[1])).toContain('"24px"')
    await fiber.dispose()
