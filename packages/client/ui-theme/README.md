...
All menus share the theme fill and blur through `MenuSurface`; platform code must preserve the token values. Explicit solid exceptions and the enforced source rules are defined in the [styling reference](../../../docs/web-styling.md#component-rules). Modal masks retain their dark translucent fill without background blur.
