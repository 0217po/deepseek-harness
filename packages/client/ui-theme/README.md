...
`brand-font.css` exports the local Montserrat Light and Regular faces (normal style, weights 300 and 400), with `montserrat-light.ttf`, `montserrat-regular.ttf` and its SIL Open Font License in `lib/styles/`. Desktop bundles the same stylesheet, font and license for offline welcome brand text; ordinary UI keeps its system font stack.
