...
The Host entry exposes `pluginInstallLocation.country()` through the generated Remote interface, using [ip-geolocation](../../util/ip-geolocation/README.md) for transport and response validation. It shares pending lookups, expires cached readings, and aborts and awaits an outstanding lookup on unload. Calls after unload return a rejected Promise.
