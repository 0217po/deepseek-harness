...
The Host entry exposes `pluginInstallLocation.country()` through the generated Remote interface, using [ip-geolocation](../../util/ip-geolocation/README.md) for transport and response validation. It shares pending lookups, expires cached readings, and aborts and awaits an outstanding lookup on unload.
...
- Country detection observes the Host fetch exit. Remote Web users may be elsewhere, and pnpm-specific proxy settings can use a different route; users can always choose the registry manually.
...
- **The registry choice is this browser's** — it lives in `localStorage`, so another browser computes its own initial recommendation; the `dsh plugin` command and the agent tool use the Host's configured registry.
...
**Runtime invariant:** No companion is published. The panel reads Host-owned facts, and the country cache stores one query result without an independently maintained projection.
