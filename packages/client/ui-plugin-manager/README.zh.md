...
Host 入口通过生成的 Remote 接口暴露 `pluginInstallLocation.country()`，使用 [ip-geolocation](../../util/ip-geolocation/README.zh.md) 处理传输和响应校验。它共享进行中的查询、让缓存按时过期，并在卸载时中止和等待未完成查询。卸载后调用会返回拒绝的 Promise。
...
- 国家检测观察 Host fetch 的出口。远程 Web 用户可能身处其他地区，pnpm 专用代理设置也可能使用不同路径；用户始终可以手动选择安装源。
...
**运行时不变式：** 不发布伴生检查。面板读取 Host 事实，国家缓存只保存一次查询结果，没有独立维护的投影。
