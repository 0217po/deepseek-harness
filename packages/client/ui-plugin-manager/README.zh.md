...
`plugins.bundle.status` 以 npm 包名为 key，在 Bundle 卡片开关前显示简短状态；`plugins.bundle.config` 提供 Bundle 详情配置。`plugins.bundle.activation` 在用户从列表显式启用后提供 Bundle 自有引导，并传入关闭引导和打开详情的回调。仅列出已启用的 Bundle 不会触发引导。
