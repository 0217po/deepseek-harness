...
设置页显示内置与自定义卡片分组、默认项高亮和点击卡片选择；没有 preset 的分组不显示。本页不编辑任何内容：标题旁的问号图标打开一个对话框，说明 preset 由已安装的 bundle 声明；当 `cordis` preset 在列表中且存在会话流程时，可从对话框启动一个创造模式任务，以 bundle 形式创建或覆盖 preset。
