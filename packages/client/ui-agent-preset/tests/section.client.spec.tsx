...
it('explains that presets come from bundles and launches Creator from the guidance', () => {
  const launch = vi.fn()
  const actions = view({ rows: [{ id: 'cordis', isDefault: true }] }, launch)
  fireEvent.click(screen.getByRole('button', { name: en.authoringHelp }))
  const dialog = screen.getByRole('dialog', { name: en.authoringTitle })
  expect(within(dialog).getByText(en.authoringBody)).toBeTruthy()
  fireEvent.click(within(dialog).getByRole('button', { name: en.creatorDraft }))
  expect(launch).toHaveBeenCalledOnce()
  expect(actions.close).toHaveBeenCalledOnce()
  expect(screen.queryByRole('dialog', { name: en.authoringTitle })).toBeNull()
})
it('offers no Creator action without the conversation flow or the cordis preset', () => {
  view({ rows: [{ id: 'cordis', isDefault: true }] })
  fireEvent.click(screen.getByRole('button', { name: en.authoringHelp }))
  expect(within(screen.getByRole('dialog')).queryByRole('button', { name: en.creatorDraft })).toBeNull()
  fireEvent.click(within(screen.getByRole('dialog')).getAllByRole('button', { name: en.close }).at(-1)!)
  cleanup()
  view({}, vi.fn())
  fireEvent.click(screen.getByRole('button', { name: en.authoringHelp }))
  expect(within(screen.getByRole('dialog')).queryByRole('button', { name: en.creatorDraft })).toBeNull()
})
it('dismisses the guidance from its header close', () => {
  view()
  fireEvent.click(screen.getByRole('button', { name: en.authoringHelp }))
  const dialog = screen.getByRole('dialog', { name: en.authoringTitle })
...
it('disables Creator in the guidance when mode selection is hidden', () => {
  const launch = vi.fn()
  view({ showPicker: false, rows: [{ id: 'cordis', isDefault: true }] }, launch)
  fireEvent.click(screen.getByRole('button', { name: en.authoringHelp }))
  const button = within(screen.getByRole('dialog')).getByRole<HTMLButtonElement>('button', { name: en.creatorDraft })
