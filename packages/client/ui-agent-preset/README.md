...
The chip follows the preset projection of its bound Session while its plugin and Provider generation remain active. A late projection or another client's preset update refreshes the display without submitting a selection; list notifications do not repeat an in-flight selection.
