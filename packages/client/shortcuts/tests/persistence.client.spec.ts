...
const definitions: readonly ShortcutDefinition[] = [{ id: one, defaults: { desktop: { ...binding, code: 'KeyB' } } },
  { id: two, defaults: { desktop: { ...binding, code: 'Comma' } } }]
...
function fixture(reread = false) {
  ...
  const storage = { read: vi.fn(async () => raw),
    write: vi.fn(async (next: string) => { raw = next }), backup: vi.fn(async (_raw: string) => {}) }
...
describe('serialized preference persistence', () => {
  it('publishes a save once and only after durable completion; rejects the competing old revision', async () => {
    const f = fixture()
    const initial = await f.store.reload()
    let release!: () => void
    let started!: () => void
  ...
  it('keeps the previous configuration and revision on write failure, allowing the same draft to retry', async () => {
    const f = fixture(); const initial = await f.store.reload()
    ...
    expect((await restarted.store.reload()).document).toEqual(f.state().document)
  })

  it.each(['{broken', '{"schemaVersion":3,"profiles":{}}'])('retains accepted bindings and original bytes after reading %s', async (raw) => {
    const f = fixture(); await f.store.reload(); await f.store.edit(edit, f.state().revision)
    const accepted = f.state().document
    f.external(raw)
    const bad = await f.store.reload()
    ...
    f.storage.backup.mockRejectedValueOnce(new Error('backup failed'))
    expect((await f.store.edit({ type: 'recover' }, bad.revision)).status).toBe('write-failed')
    expect(f.raw()).toBe(raw)
    expect((await f.store.edit({ type: 'recover' }, bad.revision)).status).toBe('saved')
    expect(f.storage.backup).toHaveBeenLastCalledWith(raw)
    expect(f.state().document.profiles).toEqual({})
  })

  it('uses defaults on the first bad read, reports access failure, and refuses recovery without original bytes', async () => {
    const f = fixture(); f.storage.read.mockRejectedValueOnce(new Error('denied'))
    expect((await f.store.reload()).usingDefaults).toBe(true)
    f.storage.read.mockRejectedValueOnce(new Error('still denied'))
    expect((await f.store.edit({ type: 'recover' }, f.state().revision)).status).toBe('unreadable')
    expect(f.storage.write).not.toHaveBeenCalled()
    f.external('{'); expect((await f.store.reload()).error).toBe('invalid')
    f.external('{"schemaVersion":1,"profiles":{}}'); expect((await f.store.reload()).status).toBe('ready')
  })

  it('rereads before a browser write and requires an explicit review of external updates', async () => {
    const f = fixture(true); const initial = await f.store.reload()
  ...
  it('accepts Windows single keys and editing combinations but rejects explicit/default conflicts', async () => {
    const f = fixture(); await f.store.reload()
...
it('continues transactions after a rejected unsupported code and contains subscriber failures', async () => {
  ...
  try {
    ...
    expect((await f.store.reload()).document).toEqual(result.snapshot.document)
