...
Mouse clicks select models and efforts. Releasing a press off the menu button cancels the click and preserves keyboard navigation. While a selection is pending, focus stays on the trigger; a rejected selection leaves the menu open, and Tab returns to the current row.
