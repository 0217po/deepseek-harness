...
export const zh: Record<SubagentSettingsLocaleKey, string> = {
  ...
  subagentDepthHelp: '限制智能体创建子智能体的递归层级。',
  subagentDepthZero: '禁用子智能体',
  subagentDepthOne: '仅允许主智能体创建子智能体',
  subagentDepthOverride: '如果某个工具单独设置了最大递归深度，以该工具的设置为准。',
  subagentMaxActive: '子智能体并行数量上限',
  subagentCapacityHelpLabel: '子智能体并行数量上限说明',
  subagentCapacityHelp: '同一主智能体下，所有递归层级同时存活的子智能体总数，主智能体不计入。达到上限时，新的启动请求会被拒绝。',
  subagentDepthInvalid: '请输入不小于 0 的整数。',
  subagentCapacityInvalid: '请输入不小于 1 的整数。',
  subagentModelSelectionTitle: '模型选择',
  subagentModelSelectionToggle: '允许智能体为子智能体选择模型',
  subagentModelSelectionChoose: '开启后，智能体可以从下方授权模型中，为每个子智能体选择提供方、模型和推理强度。仅影响新会话。',
  subagentModelSelectionAllowed: '智能体可选择的模型',
  ...
  subagentModelSelectionOff: '关闭后，子智能体使用配置的默认模型或继承父智能体的模型；已选模型会保留。',
}
