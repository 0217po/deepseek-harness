...
| `StateDot` | Solid green `done`, amber `warning`, red `error`, and scrollbar-hover-grey `idle` marks in a 10px slot, plus a dark-grey 14px rotating `ongoing` loader. `aria-hidden`, so the render site owns the name. |
