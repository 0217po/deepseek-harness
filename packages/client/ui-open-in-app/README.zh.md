...
文档标题栏在可用时显示默认应用图标。菜单列出系统为该文件注册的应用，并标记默认项。选择应用只用于本次打开，主按钮始终使用系统默认应用。文件或菜单显示状态变化时会重新查询关联应用；已取消的查询不会覆盖其他文件的结果。查询失败时菜单显示提示，默认打开和定位仍可使用。平台支持范围见 [native-command](../../util/native-command/README.zh.md)。
