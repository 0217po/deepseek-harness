...
Issue #4670 的工作草稿，尚待实现与验证。本节不描述已交付行为。
...
**计划中的滚动行为。** 在关闭自动换行的左右对比中，禁止两列在上下左右四个边缘的弹性越界，同时保留可滚动范围内的原生惯性。先在 [ReviewTab.module.css](src/client/ReviewTab.module.css) 中实际承担滚动的列容器上设置 `overscroll-behavior: none`。保留 [ReviewTab.tsx](src/client/ReviewTab.tsx) 的位置同步，包括各列自己的横向终点，以及另一侧无需滚动时长内容一侧的偏移。方案不引入滚轮事件取消、自定义惯性或共享内容宽度。先检查滚动向外围面板传递的行为，再判断是否需要额外限制。
...
- 从任一列快速滑向上、下、左、右边缘，都停在对应终点，不产生弹性位移。在到达边缘前松手仍保留原生惯性，反向操作恢复正常同步滚动。
- 两侧横向内容等宽或不等宽时均可使用，包括仅一侧需要横向滚动的对比。纵向滚动保留横向位置。单栏和自动换行视图仍可使用，双栏到达边缘时不会意外带动外围面板。
- 短标题完整显示且不渐隐。缩窄面板时，长文件名保留尾部并在左侧渐隐；加宽到路径可完整显示时渐隐消失。在前缀相同、后缀不同的文件之间切换，仍能辨认各自尾部。文件菜单仍能选择标题对应的文件，悬停可查看完整展示路径。

**验证计划。** 扩展现有[录制会话浏览器用例](../../../apps/web/tests/changed-files-turn.e2e.ts)，覆盖改动文件标题与滚动布局，并保留其滚动范围不等的回归覆盖。对不依赖浏览器布局的新标题行为补充聚焦的[组件覆盖](tests/review-tab.client.spec.tsx)。实现者必须在 Chrome 中用真实 macOS 触控板验证弹性越界与惯性，并记录浏览器版本和结果；仅设置滚动偏移或合成滚轮事件不能证明原生回弹行为。若验证桌面端，单独记录结果。实现完成后，把已验证行为写入 review tab 使用说明与相关 JSDoc，再删除本草稿。
