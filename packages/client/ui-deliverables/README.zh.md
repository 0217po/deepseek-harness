...
关闭自动换行时，两列同步纵向滚动，横向偏移同步到各自可滚动范围内。两列在双轴上都禁用弹性边缘反馈和向外层传递滚动；各列可滚动范围内的原生惯性仍由浏览器控制。两侧横向滚动条始终位于可见对比区域底部；如果只有一侧需要横向滚动，上下滚动会保留该侧的横向偏移。
...
Issue #4670 的工作上下文：文件名展示尚未实现。下述拟议的文件名行为不是已交付保证。

**已报告的复现。** 在 macOS Chrome 中打开已完成轮次的改动文件 review，改动行数与单行长度足以支持双轴滚动。鼠标停在任一列，用触控板快速滑向各个边缘。用户报告，接受操作的一列会越过边缘回弹，另一列停在滚动终点。选中文件的路径过长时还会隐藏文件名尾部；文档预览提供了期望的参考效果。

**滚动验证。** [录制会话浏览器用例](../../../apps/web/tests/changed-files-turn.e2e.ts)覆盖从任一列向四个边缘滚动时的限制、反向滚动与不等的横向范围。2026-09-21，报告问题的用户在 Chrome 中使用真实 macOS 触控板确认，边缘回弹消失，正常惯性保留。

**计划中的文件名行为。** 选中文件的标题采用文档预览的路径展示：空间足够时完整靠左显示；裁切时保留尾部字符与扩展名，且仅在左侧渐隐；目录使用弱化颜色，文件名使用正常文字颜色；悬停显示完整的展示路径。沿用 [TextPreview.tsx](../ui-sidebar-documentpreview/src/client/TextPreview.tsx) 与 [TextPreview.module.css](../ui-sidebar-documentpreview/src/client/TextPreview.module.css) 的布局和溢出测量方式。所选文件或可用宽度变化时重新判断裁切。选择器箭头、行数统计与操作按钮保持可见。若提取共享展示组件，应放入 `ui-primitives`，而不是在功能插件之间导入组件。

**验收条件。**

- 短标题完整显示且不渐隐。缩窄面板时，长文件名保留尾部并在左侧渐隐；加宽到路径可完整显示时渐隐消失。在前缀相同、后缀不同的文件之间切换，仍能辨认各自尾部。文件菜单仍能选择标题对应的文件，悬停可查看完整展示路径。

**剩余验证。** 扩展浏览器用例以覆盖文件名裁切，并对不依赖浏览器布局的新标题行为补充聚焦的[组件覆盖](tests/review-tab.client.spec.tsx)。文件名修复通过验证后，更新 review tab 使用说明与相关 JSDoc，再删除本草稿。
