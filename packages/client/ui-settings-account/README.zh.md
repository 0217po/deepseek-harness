...
侧边栏账号菜单使用公共 Menu 的表面、背景模糊、间距和菜单项排版；功能样式仅设置入口布局。设置菜单项显示外壳传入的有效组合键。关闭设置后，焦点回到侧栏账号入口。
