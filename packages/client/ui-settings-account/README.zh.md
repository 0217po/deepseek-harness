...
服务端返回后即绘制通知，包括在设置面板的半透明蒙层之下；同时要求文档可见且卡片已完成一帧绘制。卡片展示后即排队确认，用户关闭卡片同样视为已看到，因此同一张挂载的卡只确认一次。确认失败会在本次登录生命周期内按带上限的退避重试，且不会在账号界面报错。不写入浏览器存储：退出登录、切换账号或卸载插件都会丢弃待确认队列，下一次登录会展示服务端仍然下发的内容，包括上一次登录已确认过的订单。用户已看到的卡片会保留到关闭为止，即使之后的读取没有返回赠金。
...
账号登录在模型引导的凭证编辑器之前显示可关闭的弹窗。弹窗底部采用紧凑的右对齐横排操作，主操作位于末尾。侧边栏优先显示昵称，缺少昵称时显示服务端返回的脱敏手机号或邮箱；资料加载期间保持留空。 弹窗文字和侧边栏账号状态文字不可选中，复制登录链接按钮始终可用，并在每次复制成功两秒后恢复原文案。剪贴板失败时，链接原位显示“复制失败”两秒，不中断登录。等待状态提供可复制的授权链接和加载图标；超时和失败均须用户手动重试。弹窗的“复制登录链接”和设置页的“打开”链接会从当前主题快照读取 Desktop 生效配色，追加 `theme=light` 或 `theme=dark`，使两条路径都以当前应用主题打开 Platform 页面，且不改变 Platform 已保存的主题偏好。登录失败在该弹窗内呈现；退出登录失败时菜单保持展开以便重试，并且只在控制台记录失败步骤与稳定错误码。关闭等待弹窗会取消对应 Host 登录流程。侧边栏通过设置协调器重新打开同一个 API Key 编辑器。
