...
Desktop preload 标记在账号凭证已存储后启用引导。进度属于本地 Host 设置文档，不属于 Platform 账号：重启后恢复未完成的步骤，完成或跳过后不再于当前安装中重复运行。浏览器客户端不挂载此流程。API Key 是否存在由原生欢迎后端通过仅返回布尔值的 Desktop preload 桥接提供，与登录共用凭证发现逻辑。未登录但已配置模型 API Key 的安装会绕过这些页面，应用详细过程、详细用量和开启开发者工具的配置。

欢迎页必须点击“开始设置”。额度页始终展示。首次进入时若已确认有正余额，则主操作为“继续”，次操作为“去充值”。该选择在引导控制器重建前保持固定且不持久化，后到的余额结果不会改变按钮。仅在确认不存在正余额时，继续而不充值才需确认；查询中或失败时不阻止继续。用途卡片选中后描述不变。从原生充值视图返回后保留额度页，并在后台刷新账号资料，不受页面加载或支付结果影响。“下一步”仅在确认零余额时提示；额度页的“跳过”使用充值提示，答题页使用设置提示。仅选办公用途时以简洁展示完成；选择开发或两种用途时还会询问过程详情。上一步保留已有选择，任意可跳过步骤均采用详细工作过程、简洁性能与用量以及关闭开发者工具的默认值。此流程不创建工作区或示例任务。
