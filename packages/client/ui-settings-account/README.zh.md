...
赠金到账时，侧栏账号入口上方显示一条由服务端提供文案的通知，不抢占焦点。客户端仅在账号进入登录态时、以及用户点击设置页的“刷新余额”时读取未通知赠金，不做轮询。候选顺序与文案均由服务端决定，客户端只展示第一条并原样渲染其消息。

仅在设置面板关闭、文档可见且卡片已完成一帧绘制后才会展示通知。客户端先写入待确认记录，再向服务端发起确认；两步之间中断时会重试而不是再次展示同一笔赠金。用户关闭卡片同样视为已看到。记录按 Platform origin 与账号、以订单 ID 为单位写入浏览器存储，可跨重启和重新登录，账号在处理请求期间变化时不会确认上一个账号的通知。确认失败按带上限的退避重试，且不会在账号界面报错。浏览器存储不可用时，记录仅在当前页面内保留。
...
余额卡片分行展示充值余额与赠金余额。赠金行在登录态下常驻，并承载刷新操作；没有正额赠金时显示暂无可用赠金，赠金读取失败时沿用既有的不可用文案且刷新操作仍可用。不同币种分别显示；日期只在服务端自己的消息中出现。
...
- 通知文案按客户端读取时的界面语言由服务端本地化。切换语言本身不会重新读取未通知赠金，因此已在屏幕上的通知会沿用服务端先前语言下发的文案，直到用户手动刷新。
