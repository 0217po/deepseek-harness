...
The notice is painted as soon as the server offers it, including underneath the translucent settings panel, once the document is visible and the card has passed one presented frame. Displaying a card queues its acknowledgement, and closing the card also counts as seen, so one mounted card is acknowledged once. A failed acknowledgement retries with a capped backoff for the rest of the signed-in lifecycle and never surfaces as an account UI error. Nothing is written to browser storage: signing out, switching accounts, or unloading the plugin discards the pending queue, so the next sign-in displays whatever the server still offers, including an order an earlier sign-in acknowledged. A card the user has already seen stays up until it is closed, even when a later read returns no bonus.
