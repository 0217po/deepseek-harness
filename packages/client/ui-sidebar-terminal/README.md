...
Closing or replacing a terminal tab removes it immediately and ends its process in the background. Cleanup-failure notifications and their retry actions are disabled. Collapsing, switching tabs or Sessions, floating and fullscreen presentation preserve the process.

After reload, the [sidebar restores its layout](../../client/ui-sidebar-right/README.md#state) and each terminal reconnects to its saved Host identity in the original tab. Collapsed and inactive tabs do not create recovery duplicates or change selection. Host terminals without a saved tab association do not reopen automatically; the header recovery action is disabled. A missing saved process shows a localized unavailable panel with **New terminal**. Clicking it replaces the unavailable tab in place with a fresh terminal; recovery never creates that replacement automatically.
