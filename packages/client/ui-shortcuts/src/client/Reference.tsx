...
export function ShortcutsRow({ actions, t, useCatalog }: PropsRuntime<'settings.general.item'> & Store & Locale & InjectFace<ReferenceInjected>) {
  ...
  return <div className={css.setting}>
    ...
    <Tooltip disabled={!shortcut?.keys.length} label={shortcut?.keys.length ? t('hint', { label: t('view'), keys: shortcut.keys.join(' ') }) : t('view')}>
                ...
                ? target?.id === row.id
                  ...
                  : <>
                    <span className={css.rowActions}>
                      {row.binding !== null && <button type="button" className={css.iconButton} aria-label={t('remove-label', { command: row.label })}
                        disabled={busy || config.status !== 'ready'} onClick={() => { void remove(row) }}><ShortcutIcon kind="trash" /></button>}
                      <button type="button" className={css.iconButton}
                        aria-label={t('edit-label', { command: row.label })} disabled={busy || config.status !== 'ready'}
                        onClick={() => { setTarget(row) }}><ShortcutIcon kind="edit" /></button>
                    </span>
                    <button type="button" className={css.keyButton} disabled={busy || config.status !== 'ready'}
                      aria-label={t('hint', { label: row.label, keys: row.keys.length === 0 ? t('unbound') : row.keys.join(' ') })}
                      onClick={() => { setTarget(row) }}>
                      {row.keys.length === 0 ? <span className={css.unbound}>{t('unbound')}</span> : <ShortcutKeys keys={row.keys} className={css.keyBadge} />}
                    </button>
                  </>
                : <button type="button" className={css.keyButton} disabled
                  aria-label={t('hint', { label: row.label, keys: row.keys.join(' ') })}>
                  <ShortcutKeys keys={row.keys} className={css.keyBadge} />
                </button>}
            </span>
          </li>)}</ul>
        </section>
