...
`installBundle` checks GitHub repositories with `git ls-remote` before starting pnpm, using the profile directory and the installer’s Git environment and configuration. `githubConnectionTimeoutMs` defaults to 5000 ms and limits only this check, not package download or builds. A failed check prevents installation; network failures and timeouts report `failedAt: 'spec-host'` with the existing failure kind and diagnostic log. Cancellation and manager disposal stop the check and its descendants. Registry packages, paths, tarballs and other Git hosts skip this check. A reachable repository can still fail during download or bundle validation.
