...
| A synchronous callback (a stream `'data'` listener, a timer) throws an uncaught exception at any point in the process lifetime | Fatal: dispose the app and exit nonzero; the process never resumes | Fatal: dispose the app and exit nonzero; the process never resumes | Fatal: dispose the app and exit nonzero, regardless of entry id |
