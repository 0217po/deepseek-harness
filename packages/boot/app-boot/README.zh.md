...
| 同步回调（流的 `'data'` 监听器、定时器）在进程生命周期任一时刻抛出未捕获异常 | 致命错误：释放应用并以非零码退出；进程不会恢复运行 | 致命错误：释放应用并以非零码退出；进程不会恢复运行 | 致命错误：释放应用并以非零码退出，与条目 id 无关 |
