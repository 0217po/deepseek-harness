...
Host diagnostics use the `[deepseek-account]` prefix on standard output and in the Host inspector Console, and every line carries a UTC ISO `at` timestamp. Extending the single writer in protocol.ts keeps that field on new lines. They include endpoint paths, HTTP status, numeric response codes, failure stages, invalid field paths, received types and validation codes, browser URL rejection rules, and the bounded response body with credential fields, request secret values and authorization URLs removed. The body is logged for a validated envelope, whether it succeeded or failed: a success logs `response body`, a business failure logs `business rejected` and never the success line. A request credential from an authorization header or the request body is removed wherever an echoed body repeats it, however short; only a deployment cookie's own value, which may be a flag such as 1, is limited to whole-value matches so it cannot erase the digits of a server-authored date, amount or order id. Local sign-out logs one `sign-out started` line, then either `sign-out completed` or `sign-out failed` with the stage that refused it (`closed`, `cancel-attempt`, `read-grant`, `validate-record`, `validate-issuer`, `delete-grant`, `schedule-revocation`, or `publish-state`), its error code and error name. Thrown messages, request bodies and headers, and raw exception objects are excluded.
