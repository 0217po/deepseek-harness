...
Host diagnostics use the `[deepseek-account]` prefix on standard output and in the Host inspector Console. They include endpoint paths, HTTP status, numeric response codes, and failure classifications; headers, request and response bodies, authorization URLs, and raw exceptions are excluded.
