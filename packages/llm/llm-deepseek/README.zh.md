...
`deepseek-official` 仅使用配置的 API Key 引用；`deepseek-account` 仅在账号提供方允许的推理来源使用已保存的 DSH 授权。两条路由共享协议、模型和文件设置。账号凭证缺失或不适用于目标时拒绝请求并提示登录；两条路由均不回退到另一凭证。Chat 和 Files 请求拒绝重定向。退登成功会取消当前已准备提供方为 `deepseek-account` 的运行代理，包括工具执行阶段，同时保留待处理消息和独立的 API Key 任务。

DSH 账号 token 在两种推理协议和所有 Files 操作中使用 x-dsh-auth-token，不加 Bearer 前缀。API Key 保持各协议现有的认证请求头。
