...
Successful Files responses must contain valid JSON. Upload, list, retrieve, and delete parsing failures throw `INVALID_RESPONSE` with the operation and HTTP status in the message, the status in `LlmError.failure`, and the original parser error as `cause`. Body-read transport and cancellation errors retain their identity.
