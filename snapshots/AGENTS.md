...
An owner that must keep a historical generation declares its exact `sessionFormat.version` and closed migration `coverage` names in `snapshot.yml`; record and refresh never rewrite that Session fixture. Owners without this declaration write current-writer output during record and refresh; replay accepts the retained V3 baseline or the current generation, with one selected generation per owner. The [corpus policy](../scripts/session-snapshot-corpus-policy.ts) requires V3 inputs for direct upgrade coverage and explicit V0–V2 migration coverage; a writer bump alone does not refresh fixtures.
