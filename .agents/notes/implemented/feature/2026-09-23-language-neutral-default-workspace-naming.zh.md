...
让某一行不再跟随语言的动作就是重命名，无需额外存储标记：重命名对话框以屏幕上的文本作为初值，因此确认后写入的是用户选择的标题，不再与自动标题相同。

## Alternatives considered

- 在 `WorkspaceView` 上标记"从未重命名"状态（由 Host 依据 `defaultWorkspaceId` 与自动标题推导的布尔值）可以消除下述误判，但会让 39 个文件使用的传输类型多承载一项 Host 本身别无用途的展示状态。比较存储标题是同一项判断，且不增加 wire 表面。
