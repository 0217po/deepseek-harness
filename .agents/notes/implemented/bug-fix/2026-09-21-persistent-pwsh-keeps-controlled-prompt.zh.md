...
`stdin_read` 回退是行为而不只是美化：在提供方能证明前台 stdin 等待之处，交互式子进程现在返回已捕获的部分输出，而不是空转到命令期限；其他情况下调用仍会运行到 `timeoutMs`，两者都记入工具 README。由标记界定的完整输出保持不变。

loader 组合套件现在把 `idleSilenceMs` 设为 30000，位于 send 上限之上，静默无法结算任何 send，提示符就绪一旦回归，每个用例都会失败。stub 套件删除了初始化握手模式，并改为锚定部分输出中保留的提示符文本，而不是它的缺席。
