...
Clipping transcript overflow and positioning floating controls require different ancestors. An `overflow-y: hidden` ancestor becomes the nearest scroll container for sticky descendants even when readers scroll a different outer element. A turn rail or back-to-bottom button inside that clipping ancestor therefore follows transcript geometry instead of the visible conversation scrollport.
...
|---|---|---|
| Transcript clipping | `ChatView` retains the clipped `.root` inside an unclipped frame. | Constrain transcript overflow without changing floating controls' scroll ancestor. |
...
Native smooth growth keeps an outstanding target separate from the current position. Animation progress retains follow intent. Input interrupts native motion before the browser applies reader movement; the resulting actual displacement determines whether following stops or resumes. Repeated resize reports do not restart an unchanged target. Reduced motion and explicit opening positions use immediate scrolling. Outer following keeps its existing immediate behavior and sampling policy.
...
**Remove or change the transcript clipping rule.** That would alter the overflow constraint instead of separating it from control positioning. An unclipped sibling frame retains both requirements without a Portal or document-level coordinate synchronization.
