...
- Chat 在同一次渲染中按持久输入的 rpcId 隐藏回显。QueueDock 只排除匹配本地 transcript 回显的 Inbox 行；其他排队消息仍可见且可操作。
- Session 保留已入档 transcript 的本地排除身份，直到 Inbox 投影水位达到其领取序号。随后，较高序号投影优先的规则阻止旧受理行在该身份回收后重新出现。领取跟踪和取消共用已有逐提交 receipt，并覆盖两种 Inbox target；不新增 SessionSnapshot 字段、Host 事件或持久格式。
...
**user/message 到达后立即删除排除身份。** 较晚的 Inbox 受理投影会复活 Dock 行。领取水位提供必要的排序依据，不要求两条流同时发布。
