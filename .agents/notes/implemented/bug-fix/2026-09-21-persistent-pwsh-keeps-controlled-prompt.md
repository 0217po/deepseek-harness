...
The `stdin_read` fallback is behavior, not only cosmetics: where a provider proves a foreground stdin wait, an interactive child now returns captured partial output instead of spinning to the command deadline, and elsewhere the call still runs to `timeoutMs`; the tool README records both. Complete marker-delimited output is unchanged.

The loader-composition suite now sets `idleSilenceMs: 30000`, above the send bound, so silence can settle nothing and every case fails if prompt readiness regresses. The stub suite drops the initialization-handshake modes and pins the prompt text retained in partial output instead of its absence.
