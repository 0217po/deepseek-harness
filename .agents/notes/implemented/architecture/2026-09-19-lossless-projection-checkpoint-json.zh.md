...
每个有效自有键都在 domain 重新打开后保留，包括嵌套的 `__proto__` 和 `constructor` 属性。存储 JSON 表示未变，因此 domain 版本不变；本修改修正其读取器。回归测试打开真实写入器生成的 fixture，再通过 `StorageDomain` 写入、关闭并重新打开。Session 格式版本与历史代际不变。
