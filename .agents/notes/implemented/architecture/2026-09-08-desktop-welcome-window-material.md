...
The development launcher accepts `--preview-welcome` to force the entry while using the complete Desktop startup and operations. It shares the normal development home, profile, browser data, and single-instance lock. The flag affects only the initial page decision; saving and skipping use the production callbacks. Packaged startup does not activate that switch. The generated project follows declared workspace dependencies because pnpm’s hoist index alone can omit configured plugins.
