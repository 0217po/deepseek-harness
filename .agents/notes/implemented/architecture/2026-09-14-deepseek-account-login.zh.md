...
赠金通知使用 Platform 提供的文案和订单标识。已登录启动、登录成功和手动刷新余额时读取通知。设置对话框关闭后才展示侧栏通知，不进行定时轮询。客户端只在通知卡实际可见后确认订单，并在发送请求前持久化已展示订单及待确认状态。按账号和来源隔离的本地记录防止重启后重复展示；确认失败时使用带上限的指数退避重试。Platform 确认记录阻止其他设备继续获取通知，但接口不预留展示资格，因此确认前仍可能并发展示。仅获取成功不会记录为已展示。
