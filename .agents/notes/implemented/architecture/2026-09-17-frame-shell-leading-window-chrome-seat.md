...
Under the same collapsed condition the frame publishes `--dsh-frame-leading-clearance: 160px` — the inline band the traffic lights plus the seat's two controls occupy, measured from the frame's left edge. Main panels whose content reaches the top-left corner pad by it and subtract what they already inset: the conversation title row pads `max(0px, clearance - 20px)` (its header pads 20px), and the plugin manager's page head pads `max(0px, clearance - clamp(24px, 4vw, 48px) - max(0px, (100% - 960px) / 2))`, so its centred column only indents when a narrow window would actually collide.
...
- Every current and future main panel keeps the reopen and New Session controls while the sidebar is hidden, at zero per-panel cost; panels whose content reaches the top-left corner opt into `--dsh-frame-leading-clearance`.
