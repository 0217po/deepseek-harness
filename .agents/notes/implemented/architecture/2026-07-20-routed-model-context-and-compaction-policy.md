...
For proactive pressure, compaction-basic reads the latest durable request route, resolves its adapter capacity and exact-target policy, and scales ratios into a `ResolvedCompactSpec`. Ratios scale the route's *message budget* — its context window minus the output tokens one request reserves — because a provider charges the prompt and that reservation to the same window. Scaling the whole window instead can place the threshold above the point where the provider refuses the request, so the provider can reject the request before proactive pressure qualifies. The reservation is the effective envelope's `maxTokens`, else the adapter's per-request output default. Compaction-basic performs this resolution on every check, so a provider or model switch in one session changes capacity and policy immediately. An absolute retained budget that is not below the scaled threshold fails when the target capacity first makes that comparison possible.
